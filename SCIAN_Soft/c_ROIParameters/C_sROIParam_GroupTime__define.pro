...
;
; CALLING SEQUENCE:
;      	result = obj_new('C_sROIParam_GroupTime' )
;
; METHOHDS:
;_____________________________IOISIOI____________________

pro C_sROIParam_GroupTime::apply, time = time

	whParam = [(where( *(*self.pParamStruct).pNames eq 'Group Time [s]'))[0]]
	if (n_elements(time) gt 0) then begin
		...
		(*(*self.pValueStruct)[whParam[0]]).pROIParamVect = ptr_new( time )
	endif
end

pro C_sROIParam_GroupTime::setpValueStruct, pValueStruct
...
function C_sROIParam_GroupTime::init

	ROIParamStruct = {name:'Group Time',$		;  ROI Name.
	...
	ROIValueStruct = {name: 'Group Time [s]',$
								type: 'Group ROI-Parameter-Method',$
...
pro C_sROIParam_GroupTime__define
   tmp = {C_sROIParam_GroupTime, pParamStruct: ptr_new(),$
