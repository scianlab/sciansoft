...
;
; CALLING SEQUENCE:
;       result = obj_new('C_sROIParam_3DObjVolume' )
;
; METHOHDS:
;_____________________________IOISIOI____________________


pro C_sROIParam_3DObjVolume::apply, C_sROI3DGroupObj = C_sROI3DGroupObj, position = position
    ...
    if (nObjects lt 1) then begin
       ...
       *(*self.pParamStruct).pROINumberVect = C_sROI3DGroupObj->getObjectNumberVector()
       ...
       for i = 0L, nObjects-1 do paramVect[i] = n_elements( *((C_sROI3DGroupObj->get(position = i))->getpWherePoints()) )
...
function C_sROIParam_3DObjVolume::init

    ROIParamStruct = {name:'3D Object Volume',$   ;  ROI Name.
    ...
    ROIParamNames = ['3D Volume [Voxel]','3D Volume [x³]']
    ROIParamActive = [1,0]
    ROIParamMin = [0,0]
    ROIParamMax = [1,1]
    ROIParamValues = [0,0]
...
pro C_sROIParam_3DObjVolume__define
   tmp = {C_sROIParam_3DObjVolume, pParamStruct:ptr_new(),pValueStruct:ptr_new(),inherits C_sROIParam}
end