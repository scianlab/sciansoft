...
;
; CALLING SEQUENCE:
;       result = obj_new('C_sROIParam_ObjChannelIntensityRatio' )
;
; METHOHDS:
;_____________________________IOISIOI____________________
function C_sROIParam_ObjChannelIntensityRatio::getParamNames
...
pro C_sROIParam_ObjChannelIntensityRatio::apply, C_sROIGroupObj = C_sROIGroupObj, position = position, stack_tlb = stack_tlb
    ...
    if (nObjects lt 1) then begin
       ...
       *(*self.pParamStruct).pROINumberVect = C_sROIGroupObj->getObjectNumberVector()
       ...
       for i = 0, n_elements(whParam)-1 do if (whParamActive[i]) then *(*(*self.pValueStruct)[whParam[i]]).pROIParamVect = make_array(nObjects, /float, value = -1.)
         ...
         ; get original intensity images
       s_ISM_getProjectInfo, stack_tlb = stack_tlb, tPos = tPos, zPos = zPos
       widget_control, stack_tlb, get_uValue = stateStack, /no_copy
          intCh0 = (*stateStack.pImageStackInfoObject)->getSelectedImage(tPos = tPos, chPos = 0, zPos = zPos)
          intCh1 = (*stateStack.pImageStackInfoObject)->getSelectedImage(tPos = tPos, chPos = 1, zPos = zPos)
          intCh2 = (*stateStack.pImageStackInfoObject)->getSelectedImage(tPos = tPos, chPos = 2, zPos = zPos)
       ...
       for i = 0, nObjects-1 do begin
          ...
          if whParamActive[2] then (*(*(*self.pValueStruct)[whParam[2]]).pROIParamVect)[i] = mean(intCh1[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] / (intCh0[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] > 0.01)) 
            ; I-Ratio Ch0/Ch2
          if whParamActive[2] then (*(*(*self.pValueStruct)[whParam[2]]).pROIParamVect)[i] = mean(intCh0[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] / (intCh2[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] > 0.01)) 
            ; I-Ratio Ch2/Ch0
          if whParamActive[3] then (*(*(*self.pValueStruct)[whParam[3]]).pROIParamVect)[i] = mean(intCh2[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] / (intCh0[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] > 0.01)) 
            ; I-Ratio Ch1/Ch2
          if whParamActive[4] then (*(*(*self.pValueStruct)[whParam[4]]).pROIParamVect)[i] = mean(intCh1[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] / (intCh2[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] > 0.01)) 
            ; I-Ratio Ch2/Ch1
          if whParamActive[5] then (*(*(*self.pValueStruct)[whParam[5]]).pROIParamVect)[i] = mean(intCh2[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] / (intCh1[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] > 0.01)) 
            ; max I-Ratio Ch0/Ch1
          if whParamActive[6] then (*(*(*self.pValueStruct)[whParam[6]]).pROIParamVect)[i] = max(intCh0[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] / (intCh1[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] > 0.01))
            ; max I-Ratio Ch1/Ch0
          if whParamActive[7] then (*(*(*self.pValueStruct)[whParam[7]]).pROIParamVect)[i] = max(intCh1[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] / (intCh0[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] > 0.01)) 
            ; max I-Ratio Ch0/Ch2
          if whParamActive[8] then (*(*(*self.pValueStruct)[whParam[8]]).pROIParamVect)[i] = max(intCh0[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] / (intCh2[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] > 0.01)) 
            ; max I-Ratio Ch2/Ch0
          if whParamActive[9] then (*(*(*self.pValueStruct)[whParam[9]]).pROIParamVect)[i] = max(intCh2[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] / (intCh0[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] > 0.01)) 
            ; max I-Ratio Ch1/Ch2
          if whParamActive[10] then (*(*(*self.pValueStruct)[whParam[10]]).pROIParamVect)[i] = max(intCh1[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] / (intCh2[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] > 0.01)) 
            ; max I-Ratio Ch2/Ch1
          if whParamActive[11] then (*(*(*self.pValueStruct)[whParam[11]]).pROIParamVect)[i] = max(intCh2[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] / (intCh1[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] > 0.01)) 
          
          nBottomPoints = (round(n_elements(*((C_sROIGroupObj->get(position = i))->getpWherePoints())) * ((100.-topPercentage)/100.)) - 1) > 0
            ; top I-Ratio Ch0/Ch1
          if whParamActive[12] then begin
             ratioVect = intCh0[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] / (intCh1[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] > 0.01)
          ...
          if whParamActive[13] then begin
             ratioVect = intCh1[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] / (intCh0[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] > 0.01)
             ratioVect = ratioVect[sort(ratioVect)]
             (*(*(*self.pValueStruct)[whParam[13]]).pROIParamVect)[i] = mean(ratioVect[nBottomPoints:*]) 
            ; top I-Ratio Ch0/Ch2
          endif
          if whParamActive[14] then begin
             ratioVect = intCh0[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] / (intCh2[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] > 0.01)
             ratioVect = ratioVect[sort(ratioVect)]
             (*(*(*self.pValueStruct)[whParam[14]]).pROIParamVect)[i] = mean(ratioVect[nBottomPoints:*]) 
            ; top I-Ratio Ch2/Ch0
          endif
          if whParamActive[15] then begin
             ratioVect = intCh2[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] / (intCh0[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] > 0.01)
             ratioVect = ratioVect[sort(ratioVect)]
             (*(*(*self.pValueStruct)[whParam[15]]).pROIParamVect)[i] = mean(ratioVect[nBottomPoints:*]) 
            ; top I-Ratio Ch1/Ch2
          endif
          if whParamActive[16] then begin
             ratioVect = intCh1[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] / (intCh2[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] > 0.01)
             ratioVect = ratioVect[sort(ratioVect)]
             (*(*(*self.pValueStruct)[whParam[16]]).pROIParamVect)[i] = mean(ratioVect[nBottomPoints:*]) 
            ; top I-Ratio Ch2/Ch1
          endif
          if whParamActive[17] then begin
             ratioVect = intCh2[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] / (intCh1[*((C_sROIGroupObj->get(position = i))->getpWherePoints())] > 0.01)
             ratioVect = ratioVect[sort(ratioVect)]
...
function C_sROIParam_ObjChannelIntensityRatio::init

   ROIParamStruct = {name:'Object Intensity Ratio',$     ;  ROI Name.
...
pro C_sROIParam_ObjChannelIntensityRatio__define
   tmp = {C_sROIParam_ObjChannelIntensityRatio, pParamStruct:ptr_new(),pValueStruct:ptr_new(),inherits C_sROIParam}
end