function s_2Dmoments, image, order = order, fAreaNorm = fAreaNorm
  ...
  ; - Teta: Winkel zwischen x,y von image und x'y', den Hauptachsen des Objektes
  ;   : tan(2*teta) = 2*MY11/(MY20-MY02)
  ; - Rot./Transl. /Zoom- Invariante Momente MY' durch Transf. auf Hauptachsen:
  ;|x'|= |cos  sin| * |x|
  ;|y'|  |-sin cos|   |y|
  ...
  maxOri = max(image)
    ...
    ; if image = [0,0,1,3,3,2,2,2] then r = [5,7,8,11,13,0,1,2,5,6,7,3,4]
  moments_0 = histogram(image, min = 1, max = maxOri, rev = r)  ; MY00: 0'th order = Areas
  whereNoObj = where(moments_0 eq 0, complement = whereObj, ncomplement = nObj)

    ; define return vector [area, xs, ys, teta, MY10, MY01, MY11, ...]
  moments = dblArr(nObj, 4 + order * 3)

  if (maxOri ne nObj) then moments[*,0] = moments_0[whereObj] else moments[*,0] = moments_0

     ; 1st order moments in relation to [0,0]
  for i = 0, nObj-1 do moments[i,1:2] = [ total((r( r[whereObj[i]] : r[whereObj[i]+1]-1 )) mod szOri[1]) ,$
                                          total( floor( (r( r[whereObj[i]] : r[whereObj[i]+1]-1 )) / (1.*szOri[1])) ) ]
  ...
  for i = 0,nObj-1 do begin
     a = double((r[ r[whereObj[i]] : r[whereObj[i]+1]-1]) mod szOri[1]) - moments[i,1]
     aa= 1. * floor( 1. *  (r( r[whereObj[i]] : r[whereObj[i]+1]-1)) / szOri[1] ) - moments[i,2]
  ...
  for i = 0,nObj-1 do begin      ; ys -> Schwerpunktshauptachse
  ...
  ; Please Check... FASL 20131002 5.31
  ; I will try to fix  issues with individual points and/or lines with width "0"
          whereZero = where((moments[*,4] eq 0) and (moments[*,5] eq 0) , count)
          if (count gt 0) then begin
              moments[whereZero,4] = 1
              moments[whereZero,5] = 1
          end
  return, moments
end