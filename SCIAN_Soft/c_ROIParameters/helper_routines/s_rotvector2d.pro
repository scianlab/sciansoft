...
;
; PURPOSE:
;       Calculates and returns rotated vector with alfa = [0,360] counterclockwise.
;
; AUTHOR:
...
;
; CALLING SEQUENCE:
;      NewxyVectorTo = s_RotVector2D(xyVectorFrom, xyVectorTo, alfa, Unit = Unit)
;
; REQUIRED INPUTS:
;      a, b
;	   for:  a = [1,0]
...
function s_rotVector2D, xyVectorFrom, xyVectorTo, alfa, Unit = Unit

	rotVector =  xyVectorTo -xyVectorFrom
	if (keyword_set(Unit)) then rotVector = rotVector*Unit / (sqrt(total(1.*rotVector*rotVector)))

	alfa = alfa/!radeg		;angle in rad ANTICLOCKWISE.
	cosa = cos(alfa)<1.>(-1.)		; RotationMatrix | cosa  sina  |
	sina = sin(alfa)<1.>(-1.) 		;							 | -sina  cosa |

	return,   [ xyVectorFrom[0] + cosa*rotVector[0]-sina*rotVector[1],$
				   xyVectorFrom[1] + sina*rotVector[0]+cosa*rotVector[1]]
end
