pro CEDAI_histogramPlot_2010, path = path, vect = vect, name = name, veloLimitsVCL = veloLimitsVCL, linesFlag = linesFlag, tempFileName = tempFileName
...
; please update it debugging script... FASL 
    ...
    linesFlag = 1
    tempFileName = 'FASL_TEMP'

   index = where(finite(vect), count)
   if(vect[0] = -1) then begin
      vect = [0]
   endif else begin
      vect = vect(index)   
   endelse
