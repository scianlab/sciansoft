...
;
; CALLING SEQUENCE:
;       result = obj_new('NC_C_sImageFilter_SizeSelect' )
;
; METHOHDS:
...
function NC_C_sImageFilter_SizeSelect::getImageFilterType
   return, 'Single_Image_Filter_Method'
end

function NC_C_sImageFilter_SizeSelect::getImageFilterDevelopmentalState
...
function NC_C_sImageFilter_SizeSelect::apply, image = image
    if (size(image, /n_dim) ne 2) then return, image

    image = temporary(image) ne 0
    alpha = s_apop_flaeche(image, setAreaAsPixelSize = 1)
...
function NC_C_sImageFilter_SizeSelect::init
    ...
    filterParamNames  = ['Select_<=', 'Select_>=', 'Exclusive', 'Show Size Only']
    filterParamActive = [1,     1,     0, 0]
    filterParamMin    = [0,     0,     0, 0]
    filterParamMax    = [50000, 50000, 1, 1]
    filterParamValues = [0,     10000, 1, 0]
...
pro NC_C_sImageFilter_SizeSelect__define
  tmp = {NC_C_sImageFilter_SizeSelect, pParamStruct: ptr_new(), inherits C_sImageFilter}
end
