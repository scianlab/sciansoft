; $Id: orb__define.pro,v 1.5 2000/01/21 00:25:32 scottm Exp $
;
...
;
; PURPOSE:
;  This object serves as a graphical representation of an C_sOrb,
...
;               of the center of the C_sOrb, measured in data units .
;   Defaults to [0,0,0].
;  RADIUS: A floating point number representing the radius of the
;               C_sOrb (measured in data units).  The default is 1.0.
...
;  POS:   A three-element vector, [x,y,z], specifying the position
;               of the center of the C_sOrb. Defaults to [0,0,0].
;  RADIUS: A floating point number representing the radius of the
;               C_sOrb (measured in data units).  The default is 1.0.
;  DENSITY: A floating point number representing the density at which
;               the vertices should be generated along the surface of the
...
;
; EXAMPLE:
;  Create an C_sOrb centered at the origin with a radius of 0.5:
;   oOrb = OBJ_NEW('C_sOrb', POS=[0,0,0], RADIUS=0.5)
;
...
;
; Purpose:
;  Initializes an C_sOrb object.
;
;  This function returns a 1 if initialization is successful, or 0 otherwise.
;
FUNCTION C_sOrb::Init, POS=pos, RADIUS=radius, DENSITY=density, xyzFact = xyzFact,$
    ...
    IF (N_ELEMENTS(xyzFact) EQ 1) THEN  self.xyzFact = xyzFact
...
;
; Purpose:
;  Cleans up all memory associated with the C_sOrb.
;
PRO C_sOrb::Cleanup

    ; Cleanup the polygon object used to represent the C_sOrb.
...
;
; Purpose:
;  Sets the value of properties associated with the C_sOrb object.
;
PRO C_sOrb::SetProperty, POS=pos, RADIUS=radius, DENSITY=density, xyzFact = xyzFact, _EXTRA=e

    ; Pass along extraneous keywords to the superclass and/or to the
    ; polygon used to represent the C_sOrb.
...
;
; Purpose:
;  Retrieves the value of properties associated with the C_sOrb object.
;
PRO C_sOrb::GetProperty, POS=pos, RADIUS=radius, DENSITY=density,$
...
;  represent the C_sOrb.
;
PRO C_sOrb::BuildPoly
    ; Build the C_sOrb.
    ...
    FOR k=0,(nrows-1) DO BEGIN
        ...
        FOR j=0,(ncols-1) DO BEGIN
            ...
            verts[2,i] = z*self.xyzFact + self.pos[2]
    ...
    verts[2,i] = self.radius*1.0*self.xyzFact + self.pos[2]
    i = i + 1L
    bot = i
    verts[0,i] = self.pos[0]
    verts[1,i] = self.pos[1]
