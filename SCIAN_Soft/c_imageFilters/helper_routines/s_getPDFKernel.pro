...
function s_getPSFKernel, PSF = PSF, xPixSize = xPixSize, yPixSize = yPixSize
    ...
    print, 'Result of Gauss-Fit for PSF: A0*exp(-.5*((x-A1)/A2)^2)'
    ...
    print, 'r0 = exp(-2) - value = ', 2.*fitValues[2]+fitValues[1]          ;-.5*((x-A1)/A2)^2 = 2 -> x = 2*A2+A1
    ...
    for i = -kernelSize+1,kernelSize-1 do for j = -kernelSize+1,kernelSize-1 do $
       PSFKernel[i+kernelSize-1,j+kernelSize-1] = fitValues[0] * exp(-.5 * ((sqrt( (i*xPixSize - fitValues[1])^2 + (j*yPixSize - fitValues[1])^2))/fitValues[2])^2)

       ; adjust Kernel to Picture-Pixel Dimensions
    return, congrid(PSFKernel, (ceil((2*kernelSize-1) * .04/xPixSize)) > 3, (ceil((2*kernelSize-1) * .04/yPixSize)) > 3, /interp, /center)
end