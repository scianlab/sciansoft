...
;
; CALLING SEQUENCE:
;     result = obj_new('C_sImageFilter_LogicalFilter')
;
; METHOHDS:
;_____________________________IOISIOI____________________

function C_sImageFilter_LogicalFilter::getImageFilterType
    return, 'Multiple_Image_Filter_Method'
end


function C_sImageFilter_LogicalFilter::apply, image = image,$
  ...
  if (fSelf eq 0) or (fSelf eq 1) then begin
    ...
    ;if (((n_elements(mask_2) eq 1) and (mask_2[0] eq -1)) or ((n_elements(mask_2) eq 9) and (mask_2[0] eq 0))) then begin; Susana: workaround for an old&incongruent mask saved to disk.
  ...
  whParam = (where((*(*self.pParamStruct).pNames) eq '0->Copy_1->EQ_2->AND_3->OR_4->NOT_5->Grow_6->Shrink_7->Touch_8->Mult_9->XOR'))[0]
  ...
  case logOperator of
    ...
    9: begin
       max1 = max(mask_1)
       max2 = max(mask_2)
       if (max1 gt 1) then print, 'Warning: Logical Filter XOR operation requires 0-1 image values. Mask 1 will be divided by 255.'
       if (max2 gt 1) then print, 'Warning: Logical Filter XOR operation requires 0-1 image values. Mask 2 will be divided by 255.'
       return, ((max1 gt 1) ? mask_1 / 255 : mask_1) xor ((max2 gt 1) ? mask_2 / 255 : mask_2)
       endcase
    else: return, mask_1
  endcase
end
...
function C_sImageFilter_LogicalFilter::init
  ...
  filterParamNames = ['1st_Time',$
                      ...
                      '0->Copy_1->EQ_2->AND_3->OR_4->NOT_5->Grow_6->Shrink_7->Touch_8->Mult_9->XOR']

  filterParamWidgetType = make_array(n_elements(filterParamNames), /string, value = 'widget_slider')
  filterParamActive = [1, 1, 1, 1, 1, 1, 0, 1, 1]
  filterParamMin    = [0, 0, 0, 0, 0, 0, 1, 0, 0]
  filterParamMax    = [1000, 1000, 1000, 1000, 1000, 1000, 2, 8, 1]
  filterParamValues = [0, 0, 0, 0, 0, 0, 1, 0, 0]
...
pro C_sImageFilter_LogicalFilter__define
  tmp = {C_sImageFilter_LogicalFilter, pParamStruct: ptr_new(), inherits C_sImageFilter}
end
