...
; NAME:
;      C_sImageFilter_ImageErasePixel
;      based on      C_sImageFilter_ImageFillNeighbours
;
; PURPOSE:
...
;
; CALLING SEQUENCE:
;       result = obj_new('C_sImageFilter_ErasePixel' )
;
; METHOHDS:
...
function C_sImageFilter_ImageErasePixel::getImageFilterType
   return, 'Multiple_Image_Filter_Method'
end

function C_sImageFilter_ImageErasePixel::apply, image = image,$
   ...
   for i = 0, iterations-1 do begin
      ...
      0:begin
         ...
         0: begin
             ;EQUAL
             alpha =  ((intImage eq threshold) AND image)
...
;            alpha >= shift( (((shift(intImage, 0, 1) - intImage) lt 0) * image), 0, -1)  ; down
         ...
         2: begin
            ...
            alpha =  ((intImage le threshold) AND image)
...
;            alpha >= shift( (((shift(intImage, 0, 1) - intImage) gt 0) * image), 0, -1)  ; down
         ...
         5: begin
            ...
            ;alpha >= ((intImage ge threshold) AND image)
            alpha = ((intImage ge threshold) AND image)
...
function C_sImageFilter_ImageErasePixel::init
   filterStruct = {Name:'C_ImageErasePixel',$   ; Filter Name.
   ...
   filterParamActive = [0,1,1,1,1,0,0]
   filterParamMin = [0,0,0,0,-4000.0,0,0]
   filterParamMax = [100,10,1,5,40000.,0,0]
   filterParamValues = [0,0,0,2,0.,0,0]
...
pro C_sImageFilter_ImageErasePixel__define
   tmp = {C_sImageFilter_ImageErasePixel, pParamStruct:ptr_new(), inherits C_sImageFilter}
end